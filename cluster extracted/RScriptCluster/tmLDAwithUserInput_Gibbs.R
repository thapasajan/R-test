...
#topic clustering using LDA Gibbs with user input
set.seed(100)
library(tm)
...
#Gibbs cluster method

cat("\n  --Wait, the documents are being clustered\n ")
ldaCluster <-LDA(dtm,k, method="Gibbs", control=list(alpha=20/k))


ldaCluster.topics <- as.matrix(topics(ldaCluster))
write.csv(ldaCluster.topics,file=paste("LDA_Gibbs DOCs to ", k ," Cluster.csv"))


ldaCluster.terms <- as.matrix(terms(ldaCluster,6))
#write.csv(ldaCluster.terms,file=paste("LDA_Gibbs", k ,"TopicsToTerms.csv"))

#topics name for cluster
write.csv(terms(ldaCluster),file=paste("LDA_Gibbs Topic names for ", k ," clusters.csv"))
